...
# #     and answer queries with hybrid routing:
# #       - Counting questions -> SQL over full table
# #       - Everything else -> vector context + Gemini
# #     """
...
# #         prompt = dag_run_conf.get("prompt", "") or ""
...
# #         # Hybrid routing: counting questions go to SQL over the full table
# #         lower = prompt.lower()
# #         if re.search(r"\b(how many|total number|count|entries|rows)\b", lower):
# #             db_path = "/opt/airflow/data/legal_demo.duckdb"
# #             conn = duckdb.connect(db_path)
# #             total = conn.execute("SELECT COUNT(*) FROM acris_clean").fetchone()[0]
# #             answer = f"The dataset has {total:,} entries."
# #             ctx = "Computed with: SELECT COUNT(*) FROM acris_clean"
# #             ti = context["ti"]
# #             ti.xcom_push(key="answer", value=answer)
# #             ti.xcom_push(key="context", value=ctx)
# #             return answer

# #         # Otherwise: vector context + Gemini
